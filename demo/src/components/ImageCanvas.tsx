...
import type { Corner, GridCoords, Point2 } from "../types/calib-targets";
...
function cornerColor(corner: DrawableCorner, _idx: number): string {
  if (corner.id != null) {
...
export function ImageCanvas({ image, detection }: Props) {
  ...
  useEffect(() => {
    ...
    let corners: DrawableCorner[] = [];
    if (detection.mode === "chessboard" && detection.result) {
      corners = detection.result.corners;
    ...
    if (corners.length > 0) {
      const gridMap = new Map<string, DrawableCorner>();
