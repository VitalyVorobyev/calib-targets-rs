...
/// Check whether `other_angle` is aligned with `reference_angle`
/// within the given `tolerance` (all in radians).
///
/// - "Aligned" means pointing roughly along the same or opposite
///   direction as `reference_angle` (difference ≈ 0 or π).
pub fn is_aligned(reference_angle: f32, other_angle: f32, tolerance: f32) -> bool {
    let diff_abs = angle_diff_abs(reference_angle, other_angle);
    let tol = tolerance.abs();
    diff_abs <= tol || (std::f32::consts::PI - diff_abs).abs() <= tol
}
...
pub fn is_aligned_or_orthogonal(reference_angle: f32, other_angle: f32, tolerance: f32) -> bool {
    is_aligned(reference_angle, other_angle, tolerance)
        || is_orthogonal(reference_angle, other_angle, tolerance)
}
