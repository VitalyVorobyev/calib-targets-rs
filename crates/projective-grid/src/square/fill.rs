...
//! attachments. The validator's existing gates (`is_eligible`,
//! `required_label_at`, `label_of`, `accept_candidate`) are reused
//! verbatim; the edge check delegates to [`GrowValidator::fill_edge_ok`]
//! which, by default, forwards to [`GrowValidator::edge_ok`].
//!
//! # Pattern extension points
//!
//! Two optional methods on [`GrowValidator`] let pattern crates
//! customise the fill pass without touching this module:
//!
//! - [`GrowValidator::eligible_for_fill`] — widen the admissible
//!   candidate set during the booster pass (e.g. admit near-cluster
//!   corners that the precision core dropped by a hair).
//! - [`GrowValidator::fill_edge_ok`] — replace the scalar-`cell_size`
...
pub fn fill_grid_holes<V: GrowValidator>(
    ...
    for _iter in 0..params.max_iters.max(1) {
        stats.iterations += 1;
        let (tree, slot_to_corner) = build_fill_tree(positions, grow, validator);
        let ctx = FillCtx {
            positions,
            cell_size,
            params,
...
fn build_fill_tree<V: GrowValidator>(
    positions: &[Point2<f32>],
    grow: &GrowResult,
    validator: &V,
    ...
    for (idx, pos) in positions.iter().enumerate() {
        if validator.eligible_for_fill(idx) && !grow.by_corner.contains_key(&idx) {
...
struct FillCtx<'a, V: GrowValidator> {
...
fn try_fill_cell<V: GrowValidator>(
    cell: (i32, i32),
    grow: &mut GrowResult,
    ctx: &FillCtx<'_, V>,
) -> Option<usize> {
    ...
    // Required label at this cell (parity for a chessboard, or `None`
    // for patterns with no parity gate).
        ...
        .into_iter()
    {
        ...
        if let Some(req) = required_label {
            if validator.label_of(idx) != Some(req) {
                continue;
            }
        }
        if matches!(
            validator.accept_candidate(idx, cell, pred, &neighbours),
            Admit::Reject
        ) {
            continue;
        }
    ...
    // Edge-invariant check via the validator's fill-aware variant.
    // At least one cardinal labelled neighbour must accept the
    // candidate; the chessboard validator can swap the scalar
...
fn any_cardinal_fill_edge_ok<V: GrowValidator>(
    c_idx: usize,
    pos: (i32, i32),
    positions: &[Point2<f32>],
...
mod tests {
    use super::*;
    use crate::square::grow::{Admit, LabelledNeighbour};
