...
//! of `nalgebra::Point2<f32>` into `delaunator`'s `f64` `Point` format and
//! exposes the half-edge structure needed by the topological pipeline. The
//! conversion cost is negligible compared to triangulation, and the `f64`
...
/// Convert `Point2<f32>` to `delaunator::Point`. The `f64` internal type
/// buys numerical robustness for near-degenerate inputs.
#[inline]
