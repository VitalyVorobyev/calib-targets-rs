...
/// `(Hex, Oriented3)`.
///
/// Mirrors [`detect_square_oriented2_topological_all`] but on a hex point
/// lattice: the Delaunay triangles *are* the unit cells (no diagonal class,
/// no triangle-pair-to-quad merge), so classification keeps triangles whose
/// three edges align with three distinct axis families, and the walk labels
/// axial `(q, r)` coordinates by parallelogram completion. The shared back-half
/// (fit + residual drop) runs through [`LatticeKind::Hex`] / its
