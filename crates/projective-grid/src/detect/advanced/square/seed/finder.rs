...
//! parallelogram completion, edge-ratio match. Caller-specific gates
//! (feature-class labels, local-axis consistency, midpoint-violation
//! rules) plug in via the [`SquareSeedPolicy`] trait.
//!
//! # Algorithm
//!
//! 1. The policy enumerates primary diagonal candidates and secondary
//!    off-diagonal candidates.
//! 2. For each primary seed corner: KD-tree-search the secondary set
//!    for nearest neighbours, classify each by angular distance from
//!    the seed corner's two local axes, enumerate one neighbour per
//!    axis, predict the opposite diagonal corner by parallelogram
//!    completion, find the nearest primary-class candidate within
//!    `close_tol_rel × avg_edge`, verify all four edges agree pairwise
//!    on length within ratio tolerance, verify the policy's per-edge
//!    gate on every edge, and reject the quad if its midpoints / center
//!    carry a caller-defined intermediate-corner violation.
...
/// Caller-supplied policy used by [`find_quad`].
///
/// Implementations supply per-corner geometry (`position`, `axes`),
/// the two seed-candidate classes, the per-edge invariant, and the
/// midpoint-violation check.
pub trait SquareSeedPolicy {
    ...
    /// Indices eligible to act as the seed's off-diagonal corners
    /// `(1, 0)` and `(0, 1)`.
    /// Order is irrelevant (the KD-tree handles spatial search).
    fn secondary_candidates(&self) -> Vec<usize>;

    /// True iff the directed edge `from → to` satisfies the caller's
    /// per-edge invariant.
    ///
    ...
    /// True iff the seed quad has a caller-defined midpoint /
...
pub struct SeedQuadParams {
    ...
    /// `K` in the KD-tree query for secondary candidates. 32 is enough
    /// to capture grid neighbours at unknown lattice scale.
...
///
/// Returns the first quad plus mean edge length as the `cell_size`
/// that passes every pattern-agnostic geometric check and every
/// policy-supplied gate. Returns `None` when no quad
/// satisfies all constraints.
pub fn find_quad<V: SquareSeedPolicy>(policy: &V, params: &SeedQuadParams) -> Option<SeedOutput> {
    let a_indices = policy.primary_candidates();
    let bc_indices = policy.secondary_candidates();
    ...
    for &a_idx in &a_indices {
        let a_pos = policy.position(a_idx);
        let a_axes = policy.axes(a_idx);
        ...
        let mut neighbors: Vec<(usize, f32, Vector2<f32>)> = bc_tree
            .nearest_n::<SquaredEuclidean>(&[a_pos.x, a_pos.y], params.k_secondary)
            .into_iter()
            .map(|nn| {
        ...
        for (b_idx, b_dist, b_off) in b_cands.iter().take(params.top_per_axis) {
            for (c_idx, c_dist, c_off) in c_cands.iter().take(params.top_per_axis) {
                ...
                let bd = (policy.position(d_idx) - policy.position(*b_idx)).norm();
                let cd = (policy.position(d_idx) - policy.position(*c_idx)).norm();
                let all = [ab, ac, bd, cd];
                ...
                // Per-edge policy gate.
                if !policy.edge_ok(a_idx, *b_idx, params.axis_tol_rad)
                    || !policy.edge_ok(a_idx, *c_idx, params.axis_tol_rad)
                    || !policy.edge_ok(*b_idx, d_idx, params.axis_tol_rad)
                    || !policy.edge_ok(*c_idx, d_idx, params.axis_tol_rad)
                {
                    continue;
                }
                ...
                if policy.has_midpoint_violation(seed, cell_size) {
                    continue;
                }
...
mod tests {
    ...
    impl<'a> SquareSeedPolicy for ToyValidator<'a> {
        fn position(&self, idx: usize) -> Point2<f32> {
            self.positions[idx]
        }
        fn axes(&self, idx: usize) -> [LocalAxis; 2] {
    ...
    fn rejects_seeds_that_violate_edge_ok() {
        ...
        impl<'a> SquareSeedPolicy for AlwaysReject<'a> {
            fn position(&self, idx: usize) -> Point2<f32> {
                self.0.position(idx)
            }
            fn axes(&self, idx: usize) -> [LocalAxis; 2] {
            ...
            fn primary_candidates(&self) -> Vec<usize> {
                self.0.primary_candidates()
            }
            fn secondary_candidates(&self) -> Vec<usize> {
                self.0.secondary_candidates()
            }
    ...
    fn rejects_seeds_with_midpoint_violation() {
        ...
        impl<'a> SquareSeedPolicy for MidpointViolator<'a> {
            fn position(&self, idx: usize) -> Point2<f32> {
                self.0.position(idx)
            }
            fn axes(&self, idx: usize) -> [LocalAxis; 2] {
