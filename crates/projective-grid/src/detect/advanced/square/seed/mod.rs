...
//!
//! The reusable pieces are the four-corner seed quad, its edge /
//! cell-size bundle, and the 2×-spacing midpoint-violation rejection.
//! Callers provide any target-specific feature classes or edge rules
//! through [`finder::SquareSeedPolicy`].
//!
...
/// positions `(0,0), (2,0), (0,2), (2,2)` mislabelled as a unit seed).
///
...
///                       is a stronger violation signal for their
///                       feature model.
...
pub fn seed_has_midpoint_violation(
    ...
    // including unrelated local features that may legitimately fall
    // near a grid-cell midpoint without indicating a 2×-spacing seed bug.
...
/// seed-edge lengths. This keeps the emitted scale tied to the seed's
/// own geometry instead of a global feature-spacing estimate.
///
...
mod tests {
    ...
    fn midpoint_violation_detects_2x_mislabel() {
        ...
        // size 10, but an intermediate corner at
        ...
        let positions = vec![
            ...
            Point2::new(10.0, 0.0),  // 4 = intermediate corner
        ];
        let violation = seed_has_midpoint_violation(
            ...
            20.0,
            0.3,
            &[4], // feature-class candidates
            &[],  // no center-specific class
            &[],  // no all-position fallback (already caught above)
        );
        assert!(violation);
    }
    ...
    fn midpoint_violation_detects_2x_via_unclustered_fallback() {
        ...
        // is NOT in the caller-specific `on_edge_midpoint` set.
