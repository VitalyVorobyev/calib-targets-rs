...
fn catalogue() -> ParamSchema {
    ...
    let groups = vec![
        ...
        ParamGroup {
            id: "boundary_extension",
            title: "Boundary extension",
        },
        ParamGroup {
            id: "no_cluster_rescue",
            title: "No-cluster rescue",
        },
        ParamGroup {
            id: "partial_slot_flip",
            title: "Partial slot-flip fix",
        },
        ParamGroup {
            id: "refit",
            title: "Post-grow refit",
        },
        ParamGroup {
            id: "post_geometry",
            title: "Post-geometry rescue",
        },
        ParamGroup {
            id: "geometry_check",
    ...
    let fields = vec![
        ...
        field(
            ...
            "find_seed",
            "Seed edge tolerance",
            "Seed-quad edge-length window as a fraction of cell size.",
            Float,
            None,
        ),
        field(
            ...
            "find_seed",
            "Seed axis tolerance (°)",
            "Angular tolerance (deg) between a seed edge and its matched grid axis.",
            Float,
            None,
        ),
        field(
            ...
            "find_seed",
            "Seed closure tolerance",
            "Parallelogram-closure tolerance for the seed quad, as a fraction of cell size.",
            Float,
            None,
        ),
        // --- grow -----------------------------------------------------------
        ...
        // --- validate -------------------------------------------------------
        field(
            "/advanced/line_tol_rel",
            "validate",
            "Line tolerance",
            "Straight-line collinearity tolerance (fraction of scale) in the BFS validation loop.",
            Float,
            None,
        ),
        field(
        ...
        field(
            ...
            "validate",
            "Local-H tolerance",
            "Local-homography prediction residual tolerance (fraction of scale) in validation.",
            Float,
            None,
        ),
        field(
        ...
        field(
            ...
            "validate",
            "Step-deviation threshold",
            "Flag corners whose local step deviates from the labelled-set median by more than this fraction (0 = off).",
            Float,
            Some("/advanced/validate_step_aware"),
        ),
        field(
            "/advanced/max_validation_iters",
            "validate",
            "Max validation iterations",
            "Cap on the validate → blacklist → regrow retry loop.",
            Int,
            None,
        ),
        // --- boundary_extension --------------------------------------------
        field(
            ...
            "boundary_extension",
            "Local-H boundary extension",
            "Use a per-candidate local homography (vs a single global one) for boundary extension.",
            Bool,
            None,
        ),
        field(
            ...
            "boundary_extension",
            "Boundary extension K",
            "Number of nearest labelled corners used to fit each candidate cell's local homography.",
            Int,
            Some("/advanced/boundary_extension_local_h"),
        ),
        // --- no_cluster_rescue ---------------------------------------------
        field(
            "/advanced/enable_no_cluster_rescue",
            "no_cluster_rescue",
            "Enable no-cluster rescue",
            "Re-consider Strong/NoCluster corners for empty cells after boundary extension, gated on position, parity and edge invariants.",
            Bool,
            None,
        ),
        field(
            ...
            "no_cluster_rescue",
            "Rescue axis tolerance (°)",
            "Per-axis tolerance (deg) for admitting a corner during no-cluster rescue.",
            Float,
            Some("/advanced/enable_no_cluster_rescue"),
        ),
        field(
            "/advanced/no_cluster_rescue_k_nearest",
            "no_cluster_rescue",
            "Rescue K",
            "Nearest labelled corners used for the no-cluster rescue's local homography.",
            Int,
            Some("/advanced/enable_no_cluster_rescue"),
        ),
        field(
            ...
            "Position-search radius for rescue candidates, as a fraction of cell size.",
            Float,
            Some("/advanced/enable_no_cluster_rescue"),
        ),
        // --- partial_slot_flip ---------------------------------------------
        field(
            ...
            "partial_slot_flip",
            "Enable partial slot-flip fix",
            "Swap the axis slots of orphan corners whose ordering disagrees with the labelled set's parity, unblocking later rescue.",
            Bool,
            None,
        ),
        field(
            ...
            "partial_slot_flip",
            "Slot-flip K",
            "Nearest labelled corners used for the partial slot-flip fix's local homography.",
            Int,
            Some("/advanced/enable_partial_slot_flip_fix"),
        ),
        // --- refit ----------------------------------------------------------
        field(
            "/advanced/enable_post_grow_refit",
            "refit",
            "Enable post-grow refit",
            "Recompute axis-cluster centres from labelled corners and re-run extension/rescue once with the refined centres.",
            Bool,
            None,
        ),
        field(
            ...
            "refit",
            "Refit min labelled",
            "Minimum labelled corners required before the post-grow refit runs.",
            Int,
            Some("/advanced/enable_post_grow_refit"),
        ),
        field(
            "/advanced/refit_min_shift_deg",
            "refit",
            "Refit min shift (°)",
            "Minimum centre shift (deg) required to trigger the second extension/rescue pass.",
            Float,
            Some("/advanced/enable_post_grow_refit"),
        ),
        field(
            ...
            "refit",
            "Post-grow BFS regrow (destructive)",
            "Destructive: demote all labels and re-grow under refined centres. Default off — net-negative on current corners (C2 campaign).",
            Bool,
            Some("/advanced/enable_post_grow_refit"),
        ),
        field(
            ...
            "refit",
            "Post-grow BFS extend",
            "Non-destructive cardinal-neighbour BFS that extends the labelled set under refined centres without demoting existing labels.",
            Bool,
            Some("/advanced/enable_post_grow_refit"),
        ),
        // --- post_geometry --------------------------------------------------
        field(
            ...
            "post_geometry",
            "Enable post-geometry rescue",
            "After the geometry check frees cells, re-run no-cluster rescue once so freed cells can be re-filled by orphans.",
            Bool,
            None,
        ),
        // --- geometry_check -------------------------------------------------
