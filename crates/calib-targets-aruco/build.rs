...
fn main() {
    ...
    out.push_str(
        "/// Names of every built-in dictionary compiled into this crate,\n\
         /// sorted lexicographically. Pass any entry to [`builtin_dictionary`].\n",
    );
    ...
    for (name, path) in &dicts {
        ...
        out.push_str(&format!("pub(crate) static {codes_ident}: &[u64] = &[\n"));
        for v in &codes {
        ...
        out.push_str(&format!(
            "/// The `{name}` built-in marker dictionary \
             ({n}×{n} bits, {} entries).\n",
            codes.len(),
        ));
        out.push_str(&format!(
    ...
    out.push_str(
        "/// Look up a built-in dictionary by name.\n\
         ///\n\
         /// Accepts any name in [`BUILTIN_DICTIONARY_NAMES`]; returns `None`\n\
         /// for an unknown name.\n",
    );
